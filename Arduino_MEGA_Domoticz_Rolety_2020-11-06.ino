#define names "Arduino_Mega_Rolety" //Board name
#define versions "1.1.3"            //Sketch version
#define date "2020-11-06"           //Upload date
#define ipAddress "192.168.1.221"   //IpAdress only for information not set
...
#define In16 16//BlindSwitchDinneroomLeft
#define In17 17//BlindSwitchDinneroomRight
...
//########################################################################################
byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xF1 }; //physical mac address
...
unsigned long timeSwitches = 500;
//Status of connected buttons
int Blind4243 = 0;//Bedroom
int Blind4445 = 0;//Barosz
...
bool SerialPrintGet = true;
bool SerialPrintLog = true;
...
void setup() {
  ...
  //pinMode(In20, INPUT_PULLUP);//SDA
  //pinMode(In21, INPUT_PULLUP);//SCL
  ...
  Serial.print(ver);
  Serial.print(" Arduino Mega Rolety IP: ");
  ...
  Log+=ver;
  Version(165);//Informacja do Domoticza o wersji oprogramownia, dacie wgrania i IP Arduino
  sendLOG(Log, SerialPrintLog);
  sensors.begin();
}
//##########################################################PĘTLA LOOP##########################################################
    ...
    if (digitalRead(In14) == LOW) {
      if (currentSwitches[0] - previousSwitches[0] > timeSwitches) {
        switch (Blind4243) {
          ...
          break;
          case 1:
            sendGET("/json.htm?type=command&param=switchlight&idx=145&switchcmd=Stop", SerialPrintGet); delay(10);
            previousSwitches[0] = currentSwitches[0];
            delay(10);
            Blind4243 = 2;
          break;
...
void ParseCommands() {
  ...
  if(readString.indexOf("version") >0){//checks for Version
    Version(165);//idx=165 rozdzielnia główna
